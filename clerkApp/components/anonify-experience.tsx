...
type DashboardTab = "overview" | "attendees" | "photos" | "review" | "log";
type AttendeeStep = "key" | "name" | "privacy" | "done";

const tabs: Array<{ id: DashboardTab; label: string }> = [
  { id: "overview", label: "Overview" },
  { id: "attendees", label: "Opt-out attendees" },
...
export function AnonifyExperience() {
  ...
  const event = useMemo(
    () => eventList.find((item) => item.id === selectedEventId) ?? eventList[0],
    ...
    <main className="min-h-screen bg-zinc-950 text-zinc-100">
      <div className="mx-auto grid min-h-screen max-w-[1500px] gap-5 px-4 py-5 lg:grid-cols-[260px_minmax(0,1fr)_360px]">
        <aside className="rounded-lg border border-zinc-800 bg-zinc-900/80 p-4">
          ...
          <button
            className="mt-4 flex w-full items-center justify-center rounded-lg bg-cyan-300 px-3 py-2 text-sm font-semibold text-zinc-950 transition hover:bg-cyan-200"
  ...
  return (
    ...
    >
      <section
        ...
        className="w-full max-w-lg rounded-lg border border-zinc-800 bg-zinc-900 text-zinc-100 shadow-2xl"
        onClick={(event) => event.stopPropagation()}
      >
        <div className="border-b border-zinc-800 p-5">
          <h2 className="text-lg font-semibold">{createdKey ? "Event created" : "Create event"}</h2>
          <p className="mt-1 text-sm text-zinc-400">
            Generate an event key and public attendee link for opt-out submissions.
          </p>
        </div>
          ...
          {!createdKey ? (
            <>
              <label className="block text-sm font-medium text-zinc-300">
                Event name
                <input
                  className="mt-2 w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-3 text-zinc-100 outline-none focus:border-cyan-300"
                  onChange={(event) => setName(event.target.value)}
                  placeholder="HuskyHack Demo Day"
                  value={name}
                />
              </label>
              <label className="block text-sm font-medium text-zinc-300">
                ...
                <input
                  ...
                  placeholder="May 2, 2026"
                  value={date}
                />
              </label>
              <p className="rounded-lg border border-cyan-300/20 bg-cyan-300/10 p-3 text-sm text-cyan-100">
                Reference photos stay scoped to this event workspace in the demo flow.
              </p>
            </>
          ) : (
            <>
              <CopyLine label="Event key" value={createdKey} />
              <CopyLine label="Attendee link" value={`/attend?event=${createdKey}`} />
            </>
          )}
        </div>
...
function CopyLine({ label, value }: { label: string; value: string }) {
  ...
  return (
    <label className="block text-sm font-medium text-zinc-300">
      {label}
      <span className="mt-2 flex overflow-hidden rounded-lg border border-zinc-700 bg-zinc-950">
        <input
          className="min-w-0 flex-1 bg-transparent px-3 py-3 font-mono text-sm text-cyan-100 outline-none"
          readOnly
          value={value}
        />
        <button
          className="border-l border-zinc-700 px-3 text-sm text-zinc-300 hover:bg-zinc-800"
          onClick={() => {
  ...
  return (
    <section className="overflow-hidden rounded-lg border border-zinc-800 bg-zinc-950/50">
      <div className="flex items-center justify-between border-b border-zinc-800 p-4">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500">
            Mock audit trail for imports, redactions, and review handoffs.
          </p>
        </div>
      ...
      <div className="divide-y divide-zinc-800">
        {auditLog.map((entry) => (
          <div className="grid gap-3 p-4 text-sm sm:grid-cols-[90px_120px_minmax(0,1fr)]" key={entry.id}>
