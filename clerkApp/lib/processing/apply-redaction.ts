 ...
 *    Requires: Python helper running (`uvicorn ai_redaction.server:app --port 8001`)
 *    Falls back to the mock result if the helper is unreachable.
 *
 * To enable the real implementation:
 *   Set REDACTION_API_URL=http://localhost:8001 in .env.local
 *   The Python helper must be running and Pillow must be installed.
 *
 * Note: REDACTION_API_URL is a server-side env var (no NEXT_PUBLIC_ prefix).
 *   Use this only in Next.js API routes or server components, not client components.
 */
...
// ---------------------------------------------------------------------------
// Real implementation — calls Python FastAPI helper at ai_redaction/server.py
// Run helper: uvicorn ai_redaction.server:app --reload --port 8001
// ---------------------------------------------------------------------------
